...
var Store = require('fh-wfm-store');
...
module.exports = function(mediator) {
  ...
  workorders.forEach(function(workorder, index) {
    ...
    workorder.startTimestamp = newDate;
  });
  var store = new Store(mediator, 'workorder');
  store.destroy().then(function() {
    return store.create(workorders);
  }).then(function() {
    store.listen('cloud:', mediator);
  });
};
